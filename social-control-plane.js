...
function seedFirstWave(store) {
  const seedPath = path.join(__dirname, 'data', 'social-first-wave.json');
  if (!fs.existsSync(seedPath)) return { imported: 0, firstWaveCount: 0, queueSize: store.read().posts.length };
  ...
  if (imported) store.write(state);
  return { imported, firstWaveCount: firstWave.length, queueSize: state.posts.length, sourceBlobSha: seed.sourceBlobSha || null };
}
