...
window.dispatchAgent = async function(id, n) {
  ...
  // Persist proof hash run to decisions table
  const agentNames = { a: 'Path A Thermodynamic Ratchet', b: 'Path B Topological Memory', c: 'Path C RG Flow' };
  fetch(CEROS_BASE + '/api/decisions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      title: `${agentNames[id] || 'Agent ' + id} — ${n} ticks`,
      context: `ticks=${s.ticks} accepted=${s.accepted} rejected=${s.rejected}`,
      accepted: s.accepted > s.rejected,
      proof_hash: h,
      agent_name: agentNames[id] || id,
    }),
...
window.loadMetrics = async function() {
  ...
  try {
    ...
    if (tsEl) tsEl.textContent = 'Live: ' + ts;
    ...
    try { localStorage.setItem('organism_cache', JSON.stringify({ ...data, _ts: ts })); } catch(_) {}
...
async function testIntegrationHandshake() {
  ...
  try {
    const r = await fetch(CEROS_BASE + '/api/connections/test', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: selectedIntegration.id, primary: primaryVal, secondary: secondaryVal }),
    ...
    if (logOut) {
      logOut.textContent = (data.logs || []).join('\n');
      logOut.className = 'handshake-log ' + (success ? 'success' : 'error');
    }
    ...
    if (success) {
      ...
      showBanner(`✓ ${selectedIntegration.name}: ${data.message}`, 'ok');
    } else {
      ...
      showBanner(`⚠ ${selectedIntegration.name}: ${data.message}`, 'warn');
    }
  } catch (e) {
    if (logOut) {
      logOut.textContent += `\n[MCP Gateway] ✗ Network error: ${e.message}`;
      logOut.className = 'handshake-log error';
    }
    showBanner(`⚠ Connection test failed: ${e.message}`, 'err');
  }

  if (btn) { btn.textContent = 'TEST HANDSHAKE'; btn.disabled = false; }
  ...
  const statusEl = document.getElementById('active-integration-status');
  if (statusEl) {
    const connected = INTEGRATION_STATUSES[selectedIntegration.id] === 'CONNECTED';
    statusEl.textContent = INTEGRATION_STATUSES[selectedIntegration.id];
    statusEl.className = connected ? 'badge badge--green' : 'badge badge--gray';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initChecklist();
  updateWalletUI();
  // ── Instant organism restore from localStorage ────────────────────────────
  (function restoreOrganismCache() {
    try {
      const cached = JSON.parse(localStorage.getItem('organism_cache') || 'null');
      if (!cached) return;
      const orgEl = document.getElementById('cockpit-organism');
      const hlEl  = document.getElementById('cockpit-health-val');
      const agEl  = document.getElementById('cockpit-agents-val');
      ...
      const tsEl  = document.getElementById('metrics-ts');
      const rawEl = document.getElementById('cockpit-raw-val');
      if (orgEl) { orgEl.textContent = String(cached.status || 'LIVE').toUpperCase(); orgEl.className = 'cockpit-card__value green'; }
      if (hlEl)  hlEl.textContent  = cached.health        || '—';
      if (agEl)  agEl.textContent  = cached.active_agents != null ? cached.active_agents : '—';
      if (enEl)  enEl.textContent  = cached.entropy_index != null ? cached.entropy_index : '—';
      if (rvEl)  rvEl.textContent  = cached.revenue_signal != null ? cached.revenue_signal : '—';
      if (tsEl)  tsEl.textContent  = 'Cached: ' + (cached._ts || '—');
      if (rawEl) rawEl.textContent = JSON.stringify(cached, null, 2);
    } catch(_) {}
  })();

  loadMetrics();

  // Initialize modular features (guarded — defined later in file or in index.html)
  if (typeof setupIDE === 'function') setupIDE();
  if (typeof setupTerminal === 'function') setupTerminal();
  if (typeof connectTermuxWS === 'function') connectTermuxWS();
  initIntegrations();
  loadGithubIntel();
  ...
  (function startSSE() {
    let es;
    function connect() {
      es = new EventSource(CEROS_BASE + '/api/events');

      es.addEventListener('organism', (ev) => {
        try {
          const d = JSON.parse(ev.data);
          const ts = new Date().toISOString();
          // Update BRIEF / cockpit fields without a full page reload
          const orgEl = document.getElementById('cockpit-organism');
          const hlEl  = document.getElementById('cockpit-health-val');
          const agEl  = document.getElementById('cockpit-agents-val');
          const enEl  = document.getElementById('cockpit-entropy-val');
          ...
          const tsEl  = document.getElementById('metrics-ts');
          const rawEl = document.getElementById('cockpit-raw-val');
          if (orgEl) { orgEl.textContent = String(d.status || 'LIVE').toUpperCase(); orgEl.className = 'cockpit-card__value green'; }
          if (hlEl)  hlEl.textContent  = d.health         || '—';
          if (agEl)  agEl.textContent  = d.active_agents  != null ? d.active_agents  : '—';
          if (enEl)  enEl.textContent  = d.entropy_index  != null ? d.entropy_index  : '—';
          if (rvEl)  rvEl.textContent  = d.revenue_signal != null ? d.revenue_signal : '—';
          if (tsEl)  tsEl.textContent  = 'Live: ' + ts;
          if (rawEl) rawEl.textContent = JSON.stringify(d, null, 2);
          // Persist so next load is instant
          try { localStorage.setItem('organism_cache', JSON.stringify({ ...d, _ts: ts })); } catch(_) {}
        } catch (_) {}
      });
...
// ── LOCAL SOVEREIGN PERSONA ───────────────────────────────────────────────
// Instant responses for common queries — no OpenAI needed
function sovereignLocalResponse(prompt) {
  const p = prompt.toLowerCase().trim();

  const cached = (() => { try { return JSON.parse(localStorage.getItem('organism_cache') || 'null'); } catch(_) { return null; } })();
  const status  = cached?.status        || 'LIVE';
  const health  = cached?.health        || '99.9%';
  const agents  = cached?.active_agents ?? 3;
  const entropy = cached?.entropy_index ?? 0.034;
  const revenue = cached?.revenue_signal ?? 3.14;

  if (/\b(status|organism|system)\b/.test(p)) {
    return `SOVEREIGN ORGANISM STATUS\n─────────────────────────\nStatus:         ${status}\nHealth:         ${health}\nActive Agents:  ${agents}\nEntropy Index:  ${entropy}\nRevenue Signal: $${revenue}\nLast Poll:      ${cached?._ts || 'unknown'}`;
  }
  if (/\bentropy\b/.test(p)) {
    return `Entropy Index: ${entropy}\nCurrently ${entropy < 0.05 ? 'within nominal bounds — system stable.' : 'elevated — monitor agent loop variance.'}`;
  }
  if (/\b(agent|agents)\b/.test(p)) {
    return `Active Agents: ${agents}\nAll agents operating within designated parameters. Proof hashes logged to decisions ledger.`;
  }
  if (/\brevenue\b/.test(p)) {
    return `Revenue Signal: $${revenue}\nSignal is ${revenue > 3 ? 'positive' : 'below baseline'} — organism loop contributing.`;
  }
  if (/\bhealth\b/.test(p)) {
    return `System Health: ${health}\nAll subsystems nominal. No anomalies detected in current orbit.`;
  }
  if (/\b(help|what can you do|commands|capabilities)\b/.test(p)) {
    return `SOVEREIGN LOCAL COMMANDS\n─────────────────────────\nask status          — live organism status\nask entropy         — entropy index reading\nask agents          — active agent count\nask revenue         — revenue signal\nask health          — system health\nls / pwd / date     — real shell commands\n\nFor full AI: add OpenAI credits at platform.openai.com/settings/organization/billing`;
  }
  if (/\b(hello|hi|who are you|what are you)\b/.test(p)) {
    return `SOVEREIGN v3.1 — Autonomous AI Control System\nSovereign AI Studios — SIA-v6 Control\nType "ask help" for available commands.`;
  }
  if (/\b(deploy|vercel|url|domain)\b/.test(p)) {
    return `Deployment: Vercel configuration in progress.\nDomains: sovereign-ai-studios.com / sovereignaistudios.com\nLocal dev server: active on replit.dev`;
  }

  return null; // null = route to OpenAI
}
...
async function executeTerminalCommand(cmdText) {
  ...
  if (substrateTarget === 'local') {
    ...
    } else if (cmd === 'ask' || cmd === 'ai' || cmd === 'sovereign') {
      // Route AI prompts to /api/ai/chat, with local persona fallback
      const prompt = args.slice(1).join(' ');
      if (!prompt.trim()) {
        appendTerminalLine('Usage: ask <your prompt here>', 'info');
        appendTerminalLine('  ask status          — organism live status', 'info');
        appendTerminalLine('  ask what can you do — capability list', 'info');
        return;
      }
      appendTerminalLine('$ ' + cmdText, 'info');

      // Local Sovereign persona — instant, no OpenAI needed
      const localReply = sovereignLocalResponse(prompt);
      if (localReply) {
        const lines = localReply.split('\n');
        for (const line of lines) appendTerminalLine(line, 'output');
        return;
      }

      appendTerminalLine('⟳ Sovereign thinking…', 'info');
      try {
        const r = await fetch(CEROS_BASE + '/api/ai/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const data = await r.json();
        if (data.error) {
          if (data.error.includes('no credits') || data.error.includes('429') || data.error.includes('quota')) {
            appendTerminalLine('⚠ OpenAI quota exceeded. Add credits at platform.openai.com/settings/organization/billing', 'error');
            appendTerminalLine('  Local Sovereign mode still active — try: ask status, ask entropy, ask agents', 'info');
          } else {
            appendTerminalLine('AI Error: ' + data.error, 'error');
          }
        } else {
          const lines = (data.reply || '').split('\n');
          for (const line of lines) appendTerminalLine(line, 'output');
          if (data.usage) appendTerminalLine(`[tokens: ${data.usage.total_tokens}]`, 'info');
        }
      } catch (e) {
        appendTerminalLine('AI request failed: ' + e.message, 'error');
      }
      return;
    } else {
      // Route unknown commands to the real API exec endpoint
      appendTerminalLine('$ ' + cmdText, 'info');
      try {
        const r = await fetch(CEROS_BASE + '/api/exec', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (data.error) {
          ...
          if (data.hint) appendTerminalLine(data.hint, 'info');
        } else {
          if (data.stdout) appendTerminalLine(data.stdout.trimEnd(), 'output');
          if (data.stderr) appendTerminalLine(data.stderr.trimEnd(), 'error');
          if (!data.stdout && !data.stderr) appendTerminalLine('[no output]', 'info');
        }
      } catch (e) {
        appendTerminalLine('Exec error: ' + e.message, 'error');
      }
    }
    return;
  }
...
document.getElementById('terminal-input')?.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    ...
    if (!val) return;
    const cmdList = ['help', 'clear', 'sysinfo', 'ls', 'cat', 'write', 'rm', 'run', 'theme', 'ask', 'ai', 'sovereign'];
...
// ================================================================
// MISSING INIT WRAPPERS — wired up from DOMContentLoaded
// ================================================================

function setupIDE() {
  // Wire up editor textarea events
...
function setupTerminal() {
  // Terminal input keydown listener already registered at file bottom.
  // Ensure the substrate selector buttons are wired up.
  document.querySelectorAll('.selector-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const target = btn.id.replace('sub-', '');
      window.setSubstrateTarget(target);
    });
  });
}

function connectTermuxWS() {
  // Auto-connect is opt-in — the user initiates via the TERMINAL tab.
  // Just surface a ready message so the panel feels live.
  appendTerminalLine('Terminal substrate ready. Select a target above to connect.', 'info');
}

// ================================================================
// INTEL — GitHub live activity feed
// ================================================================
window.loadGithubIntel = async function() {
  const el = document.getElementById('intel-github-content');
  if (!el) return;
  el.innerHTML = '<div style="color:var(--dim);font-size:11px;padding:8px 0;">Fetching GitHub data…</div>';

  try {
    const r = await fetch(CEROS_BASE + '/api/github/intel', { signal: AbortSignal.timeout(10000) });
    ...
    if (data.user) {
      html += `<div class="onto-row"><span>Account</span><span class="green">@${data.user.login}</span></div>`;
      html += `<div class="onto-row"><span>Public Repos</span><span class="green">${data.user.public_repos}</span></div>`;
      html += `<div class="onto-row"><span>Followers</span><span class="green">${data.user.followers}</span></div>`;
    }

    if (data.commits && data.commits.length) {
      html += `<div class="intel-card__title" style="margin-top:12px;font-size:10px">RECENT COMMITS — ${data.repo}</div>`;
      data.commits.forEach(c => {
        const d = c.date ? new Date(c.date).toLocaleDateString() : '';
        html += `<div class="vault-row ok" style="flex-direction:column;align-items:flex-start;gap:2px;padding:6px 0;border-bottom:1px solid var(--border);">
          <span style="color:var(--green);font-family:var(--font-mono);font-size:10px;">${c.sha}</span>
          <span style="font-size:11px;color:var(--fg)">${c.message || '(no message)'}</span>
          <span style="font-size:10px;color:var(--dim)">${c.author} · ${d}</span>
        </div>`;
      });
    }

    if (data.repos && data.repos.length) {
      html += `<div class="intel-card__title" style="margin-top:12px;font-size:10px">REPOS</div>`;
      data.repos.slice(0, 5).forEach(r => {
        html += `<div class="onto-row"><span class="mono" style="font-size:10px">${r.name}</span><span style="color:var(--dim);font-size:10px">${r.language || '—'} ★${r.stargazers_count}</span></div>`;
      });
    }

    el.innerHTML = html || '<div style="color:var(--dim);font-size:11px">No data returned.</div>';
  } catch (e) {
    el.innerHTML = `<div style="color:var(--red);font-size:11px">GitHub fetch failed: ${e.message}</div>`;
  }
};
